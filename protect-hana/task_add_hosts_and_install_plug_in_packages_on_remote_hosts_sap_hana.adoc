...
+
link:../protect-hana/task_configure_gMSA_on_windows_server_2012_or_later.html["在 Windows Server 2016 或更高版本上為 SAP HANA 配置群組託管服務帳戶"^]



.關於此任務
* 您不能將SnapCenter伺服器作為插件主機新增至另一個SnapCenter伺服器。
* 為了讓 SAP HANA 系統複製發現主系統和輔助系統上的資源，建議使用 root 或 sudo 使用者新增主系統和輔助系統。
...
NOTE: SAP HANA插件安裝在HDBSQL客戶端主機上，該主機可以是Windows系統，也可以是Linux系統。
...
您必須在此主機上設定 HDBSQL 用戶端和 HDBUserStore。
...
安裝路徑
 a| 
SAP HANA插件安裝在HDBSQL客戶端主機上，該主機可以是Windows系統，也可以是Linux系統。
...
. 監控安裝進度。
+
特定於安裝的日誌檔案位於 /custom_location/snapcenter/logs。
