...
summary: 當您準備備份或複製資源時，您可以查看主儲存和輔助儲存上所有備份和複製的圖形表示。在拓樸頁面中，您可以看到所選資源或資源組可用的所有備份和複製。您可以查看這些備份和克隆的詳細信息，然後選擇它們來執行資料保護操作。 
---
= 在拓撲頁面中查看相關備份和克隆
:allow-uri-read: 
:icons: font
...
[role="lead"]
當您準備備份或複製資源時，您可以查看主儲存和輔助儲存上所有備份和複製的圖形表示。在拓樸頁面中，您可以看到所選資源或資源組可用的所有備份和複製。您可以查看這些備份和克隆的詳細信息，然後選擇它們來執行資料保護操作。
...
+
** 顯示的備份數量包括從輔助儲存中刪除的備份。例如，如果您使用僅保留 4 個備份的策略建立了 6 個備份，則顯示的備份數量為 6。
...
+
如果資源受保護，則顯示所選資源的拓樸頁面。

. 查看摘要卡以了解主儲存和輔助儲存上可用的備份和複製數量的摘要。
+
摘要卡部分顯示備份和克隆的總數。僅適用於 Oracle 資料庫，「摘要卡」部分也會顯示日誌備份的總數。
...
NOTE: 您無法重新命名或刪除輔助儲存系統上的備份。

. 如果要刪除克隆，請從表中選擇克隆，然後按一下image:../media/delete_icon.gif["刪除圖示"]刪除克隆。


.顯示主儲存體上的備份和克隆的範例
image:../media/topology_backups_and_clones_primary_storage.gif["主儲存中的備份和克隆的拓撲視圖。"]
