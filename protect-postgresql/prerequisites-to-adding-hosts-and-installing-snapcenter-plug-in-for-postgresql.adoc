...
summary: 在新增主機和安裝插件包之前，您必須完成所有要求。  SnapCenter Plug-in for PostgreSQL 可在 Windows 和 Linux 環境中使用。 
---
...
要在 PostgreSQL 的SnapCenter插件上執行補充命令，必須將其包含在 _allowed_commands.config_ 檔案中。
