...
summary: '在使用 PowerShell cmdlet 進行備份和還原之前，必須建立儲存虛擬機器 (SVM) 連線和憑證。' 
---
= 使用 PowerShell cmdlet 為 Exchange Server 建立儲存系統連線和憑證
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
在使用 PowerShell cmdlet 進行備份和還原之前，必須建立儲存虛擬機器 (SVM) 連線和憑證。
...
-Timeout 60
----
. 使用以下方式建立新的運行方式帳戶 `Add-Credential`命令。
+
此範例使用 Windows 憑證建立一個名為 ExchangeAdmin 的新運行方式帳戶：
